...
/* 양도·양수.
   보내는 쪽(개체 화면의 '양도')은 만들었고, 받는 쪽(/care/transfer.html)은
   아직입니다. 지금 켜면 링크를 만들 수는 있는데 받는 사람이 열 곳이
   없습니다 — 그건 기능이 아니라 함정입니다.

   'off'      아무에게도 안 보입니다.
   'premium'  프리미엄 회원만 링크를 만들 수 있습니다. 지금은 여기입니다.
   'all'      로그인한 누구나.

   ⚠️ 이건 **보내는 쪽** 만 정합니다. 받는 쪽(/care/transfer.html)은 등급을
      묻지 않습니다 — 물으면 프리미엄 회원이 무료 회원에게 개체를 넘길 수
      없게 되고, 그건 파는 사람을 막는 셈입니다. 받는 쪽은 개체 상한만
      봅니다(accept_animal_transfer 안에서 서버가 셉니다).

   ⚠️ 끄면 새 링크를 못 만들 뿐, 이미 나간 링크는 계속 열립니다. 켜 둔
      동안 보낸 링크가 갑자기 죽으면 그게 더 나쁩니다. */
var TRANSFER_ACCESS = 'premium';
