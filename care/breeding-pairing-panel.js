  ...
  window.createBreedingPairingPanel = function (deps) {
    ...
    function pairForm(p) {
      ...
      return '<div class="pad"><div class="lbl">' + (p.id ? '페어링 수정' : '페어링 등록') + '</div>'
        ...
        + '<div class="row2"><div><div class="lbl2">수컷</div><select class="in" id="p_m" aria-label="수컷">'
    ...
    function calculationCard(calculation) {
      ...
      return '<div class="note info">' + icon('bi-calculator')
        + '<div><b>계산기에서 가져온 결과</b><div class="hint">부모 A: '
        + esc(calculation.parents.A.label) + '<br>부모 B: ' + esc(calculation.parents.B.label) + '</div>'
        + rows + extra
