...
-- 두 가지를 알아서 처리합니다.
--
--   ① 개체 id 를 손으로 안 넣어도 됩니다. 사진이 있는 내 개체 중 하나를 고릅니다.
...
--      admins 표의 첫 관리자 메일로 JWT 를 흉내 내 같은 트랜잭션에서만 통과시킵니다.
...
declare
  ...
  res jsonb;
begin
  select lower(email) into em from public.admins order by email limit 1;
  if em is null then
    raise exception 'admins 표가 비어 있습니다 — 관리자 메일이 등록돼 있어야 합니다.';
  end if;

  /* JWT 흉내. 이 트랜잭션 안에서만 유효합니다. */
  perform set_config('request.jwt.claims', json_build_object('email', em)::text, true);
