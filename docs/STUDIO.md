...
### 아직 안 된 것
- **크레스티드 모프 DB 관리** — 레오파드는 `morphs`/`combos` 테이블에서
  이름·색·이미지를 고칠 수 있지만, 크레스티드 데이터는 `crested-core.js` 안에
  상수로 들어 있습니다. 관리자에서 고치려면 크레스티드용 스키마(3중 대립인자
  CS 로커스, 치사 조합 등 구조가 달라 레오파드 테이블을 그대로 못 씀)를 먼저
  설계해야 합니다.
