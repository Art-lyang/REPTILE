# 량 스튜디오 — 구조와 관리자 페이지 계획

여러 서비스를 한 도메인 아래로 모으면서 정리한 문서입니다.
새 서비스를 추가하거나 관리자 기능을 손볼 때 **먼저 이 문서를 읽고**, 바꾼 내용을 반영해 주세요.

- 최초 작성 2026-07-27
- 도메인 `ryangstudio.com` · 호스팅 Cloudflare Pages · 백엔드 Supabase

> `[구현]` 코드에 반영됨 · `[설계]` 결정됐으나 미구현 · `[검토]` 아직 결정 안 됨

---

## 1. 주소 구조

```
ryangstudio.com/            스튜디오 허브 (포트폴리오)
ryangstudio.com/gecko/      레오파드 게코 모프 계산기
ryangstudio.com/gecko/#admin  관리자
pygmytest.com/              피그미다람쥐 모프 테스트 (별도 도메인 유지)
```

### 왜 게코를 하위 경로에 두는가

**같은 출처(origin)라야 로그인이 한 번으로 통하기 때문입니다.**
Supabase 세션은 브라우저 저장소에 origin 단위로 저장됩니다.
`ryangstudio.com` 과 `art-lyang.github.io` 는 다른 origin 이라 세션이 공유되지 않습니다.
앞으로 추가할 도구도 **모두 `ryangstudio.com/<이름>/` 아래**에 두어야 계정이 하나로 유지됩니다.

### 피그미는 왜 옮기지 않는가

`pygmytest.com` 은 분양·사육 관련 검색 키워드로 SEO 자산이 쌓여 있습니다.
도메인을 옮기면 그 순위가 흔들립니다. 대신 허브에서 상호 링크만 겁니다.

**단, 피그미도 Supabase 는 같은 프로젝트를 쓸 수 있습니다.**
Supabase 는 API 호출이라 origin 제약이 없습니다.
즉 **로그인은 공유 못 해도 통계·회원 데이터는 한 곳에 모을 수 있습니다.**

---

## 2. 관리자 페이지 — 현재와 목표

### 지금 `[구현]`

게코 계산기 안에 있습니다 (`gecko/index.html` 의 `#admin`).
탭은 한눈에 · 회원 · 통계 · 코드 · 모프 · 콤보 6개이고, **전부 게코 기준**입니다.

| 탭 | 다루는 것 | 서비스 구분 |
|---|---|---|
| 한눈에 | 접속·회원·코드·브리딩 요약 | 없음 (게코 전용) |
| 회원 | profiles 목록·동의·CSV | 스튜디오 공용 (계정은 하나) |
| 통계 | visits, top_combos | 없음 (게코 전용) |
| 코드 | access_codes 발급·차단 | 게코 전용 |
| 모프·콤보 | morphs, combos | 게코 전용 |

### 문제

서비스가 늘면 **어느 서비스의 숫자인지 구분이 안 됩니다.**
지금 `visits` 테이블에는 어떤 사이트에서 온 접속인지 표시가 없습니다.

---

## 3. 해결 방향 `[설계]`

### 3.1 모든 로그에 `service` 값을 붙인다

가장 먼저 할 일입니다. 나중에 붙이면 **그 전 데이터는 영영 구분할 수 없습니다.**

```sql
alter table public.visits        add column if not exists service text default 'gecko';
alter table public.combo_queries add column if not exists service text default 'gecko';
```

각 서비스는 자기 이름을 함께 보냅니다.

| 값 | 서비스 |
|---|---|
| `gecko` | 레오파드 게코 모프 계산기 |
| `crested` | 크레스티드 모프 계산기 (준비 중) |
| `fattail` | 펫테일(아프리카 펫테일) 모프 계산기 |
| `pygmy` | 피그미다람쥐 모프 테스트 |
| `care` | 케어 관리 (아래 9장) |
| `studio` | 허브 페이지 |

`gecko-core.js` 에 `SERVICE_ID` 상수를 두고 `logVisit()` 에서 함께 전송합니다.

### 3.2 관리자 페이지를 허브로 옮긴다

게코 안에 있는 관리자는 게코가 사라지면 같이 사라집니다.
**`ryangstudio.com/admin/` 으로 분리**하고, 서비스별 탭을 두는 구조로 바꿉니다.

```
/admin/               한눈에 (전 서비스 통합)
/admin/#members       회원 (공용)
/admin/#gecko         게코 — 모프·콤보·코드·통계
/admin/#crested       크레스티드
/admin/#pygmy         피그미 — 통계만 (콘텐츠는 별도 저장소)
```

**주의** — 관리자 페이지를 옮겨도 `is_admin()` 기반 권한은 그대로 씁니다.
관리자 판정은 화면이 아니라 **DB 정책(RLS)** 에서 이뤄지므로,
페이지를 옮긴다고 보안이 약해지지 않습니다.

### 3.3 한눈에 탭 — 서비스별로 나눠 보여준다

```
전체
  오늘 접속 / 7일 / 누적          (service 무관 합계)
  전체 회원 / 신규 / 마케팅 동의   (공용)
서비스별
  🦎 게코    접속 · 등록 개체 · 프리미엄
  🐿 피그미  접속 · 테스트 완료 수
  🦎 크레스티드 …
```

---

## 4. 피그미를 관리자에 붙이려면 `[설계]`

지금 피그미는 GA4 만 씁니다. 관리자에서 보려면 **Supabase 로도 함께 기록**해야 합니다.

1. `pygmytest.com` 의 `index.html` 에 Supabase 클라이언트 추가
2. 테스트 시작·완료 시 `visits` / 결과 로그를 `service='pygmy'` 로 전송
3. 관리자 [피그미] 탭에서 조회

**주의할 것** — 피그미는 로그인이 없으므로 개인 식별 정보를 보내면 안 됩니다.
기기 식별값과 결과 모프 정도만 남기고, 개인정보처리방침에도 반영해야 합니다.

> GA4 를 그대로 두고 관리자에서는 안 보는 선택지도 있습니다.
> 지금 규모에서는 GA4 만으로 충분할 수 있으니, 굳이 서두를 필요는 없습니다. `[검토]`

---

## 5. 이전 시 함께 바꿔야 하는 것 `[중요]`

도메인이 바뀌면 **인증이 먼저 깨집니다.** 배포 전에 반드시 처리하세요.

| 위치 | 바꿀 내용 |
|---|---|
| Supabase → Authentication → URL Configuration | **Site URL** 을 `https://ryangstudio.com/gecko/` 로 |
| 같은 화면 → Redirect URLs | `https://ryangstudio.com/**` 추가 |
| Google Cloud Console → 사용자 인증 정보 | **승인된 자바스크립트 원본** 에 `https://ryangstudio.com` 추가 |
| Google — 승인된 리디렉션 URI | **그대로 둡니다.** Supabase 콜백 주소라 도메인과 무관합니다 |

기존 `art-lyang.github.io` 항목은 **당장 지우지 마세요.**
옛 주소로 들어오는 사용자가 남아 있을 수 있으니, 한동안 둘 다 열어두는 편이 안전합니다.

### 이미 반영한 것 `[구현]`

- 결과 이미지 워터마크 `art-lyang.github.io/REPTILE` → `ryangstudio.com/gecko`
- 허브·사이트맵의 링크를 `/gecko/` 로
- `_redirects` 로 `/REPTILE/*` → `/gecko/*` 301 (옛 경로 링크 대비)

---

## 6. 배포 방법 (Cloudflare Pages)

깃 없이 **폴더를 끌어다 놓는 방식**입니다.

```
ryangstudio-deploy/
  index.html          허브
  robots.txt  sitemap.xml  ads.txt  _redirects
  gecko/
    index.html  gecko-core.js  breeding.html  login.html  terms.html
    crested.html  crested-core.js
```

1. [dash.cloudflare.com](https://dash.cloudflare.com) → Workers & Pages → Create → Pages → **Upload assets**
2. `ryangstudio-deploy` 폴더를 드래그
3. Custom domains 에서 `ryangstudio.com` 연결 (네임서버를 Cloudflare 로 옮기면 자동 설정)

수정할 때마다 같은 방식으로 다시 올리면 됩니다.
나중에 자동 배포가 필요하면 GitHub 저장소를 연결하는 방식으로 바꿀 수 있습니다.

> `CNAME` 파일은 GitHub Pages 전용이라 이 폴더에는 넣지 않았습니다.

---

## 7. 작업 순서 제안

1. **Cloudflare Pages 에 올리고 도메인 연결** — 먼저 떠야 나머지를 확인할 수 있음
2. **Supabase · Google OAuth 주소 갱신** (5장) — 안 하면 로그인이 안 됨
3. **`service` 컬럼 추가** (3.1) — 늦을수록 손해
4. 관리자 페이지를 `/admin/` 으로 분리 (3.2)
5. 한눈에 탭을 서비스별로 분리 (3.3)
6. 크레스티드 계산기 완성 후 허브에 추가
7. 피그미 Supabase 연동 여부 결정 (4장)

---

## 변경 이력

| 날짜 | 내용 |
|---|---|
| 2026-07-27 | 최초 작성. 주소 구조 확정, 관리자 확장 계획, 이전 체크리스트 |

---

## 접속 통계 · 방문자 종류 (2026-07-28)

### 파일 구성
- `assets/studio-config.js` — Supabase 접속 정보. 예전에는 `gecko/gecko-core.js`
  안에만 있어서 그 파일을 읽지 않는 크레스티드는 접속 기록을 남기지 못했습니다.
- `assets/analytics.js` — 접속·조합 기록과 사람/수집기 판별. 두 계산기가 공용.
- `crested/crested-app.js` — 크레스티드의 백엔드 연결. `calculate()` 를 감싸서
  계산 결과는 그대로 두고 기록만 덧붙입니다.
- `fattail/fattail-app.js` — 펫테일도 같은 방식입니다. 다만 **관리자에서 모프를
  고치는 부분(`ft_*` 표)은 아직 없습니다.** 접속·조합 로그와 문구(ui_texts ·
  update_notes)만 붙어 있습니다. 나중에 admin 을 붙일 때 `crested-app.js` 의
  두 번째 블록과 `supabase_v4.sql` 의 `cr_*` 표를 `ft_*` 로 복제하면 됩니다.

새 도구를 추가할 때: `studio-config.js` → 코어에서 `SERVICE_ID` 정의 →
`analytics.js` 순서로 읽히게 하고, `STUDIO_SERVICES` 에 이름을 등록하면
관리자 화면의 도구 선택에 자동으로 나타납니다.

### 방문자 종류 판별의 한계 ⚠️
`analytics.js` 는 브라우저에서 도는 코드입니다. **JS 를 실행하지 않는 수집기는
이 코드까지 오지 않으므로 아예 집계되지 않습니다.** GPTBot·Googlebot 대부분이
여기 해당합니다. 지금 잡히는 것은 헤드리스 브라우저로 JS 까지 실행하는 수집기와
User-Agent 를 감추지 않는 자동화 도구뿐입니다.

전부 세려면 Cloudflare Worker 에서 요청을 받을 때 기록해야 합니다. 지금은 정적
자산만 있는 Worker 라 `fetch` 핸들러가 없고, 이를 추가하면 배포 구조가 바뀝니다.
필요해지면 그때 작업합니다.

### 크레스티드 모프 관리 (supabase_v4.sql)

표가 넷입니다. 레오파드의 `morphs` 하나로 안 되는 이유:

| 표 | 쓰임 |
|---|---|
| `cr_genes` | 유전자. 이름 4개 언어, 슈퍼폼·헷 이름, 검증 수준, 색, 사진 |
| `cr_genos` | 유전형별 이름. **카푸치노·세이블이 같은 자리**라 nn/Cn/Sn/CC/SS/CS 여섯 가지에 각각 이름이 붙습니다(루왁 등). 레오파드에는 없는 구조입니다 |
| `cr_traits` | 라인브리딩 형질. 확률 계산 대상이 아니고 그룹으로 묶여 표시만 됨 |
| `cr_combos` | 여러 유전자가 만나야 생기는 이름 (프라푸치노 등) |

**유전 구조는 DB 에 두지 않았습니다.** 대립유전자 목록이나 계산 규칙을 화면에서
잘못 고치면 확률이 조용히 틀리는데, 그건 쓰는 사람이 알아챌 수 없는 오류입니다.
구조를 바꾸려면 `crested-core.js` 를 고쳐 배포하세요. 관리자에서 고치는 것은
이름·색·사진·표시 순서·노출 여부입니다.

표가 비어 있으면 계산기는 내장 데이터를 그대로 씁니다. 관리자의
**현재 앱 목록 가져오기**를 눌러야 내장 값이 DB 로 들어갑니다.

#### 관리자에서 crested-core.js 를 왜 fetch 로 읽나
`gecko-core.js` 와 `crested-core.js` 가 둘 다 최상위에서 `const SERVICE_ID` 를
선언합니다. `<script>` 로 같이 두면 *Identifier 'SERVICE_ID' has already been
declared* 로 페이지 전체가 죽습니다. 그래서 `admin-crested.js` 는 파일을
텍스트로 받아 함수 안에서 실행하고 필요한 값만 꺼냅니다.

### 아직 안 된 것
- 크레스티드 유전자를 관리자에서 **새로 추가**하면 구조 필드가 없어 가장 흔한
  형태(bi · incdom)로 잡힙니다. 열성이나 3중 대립인자를 추가하려면
  `crested-core.js` 를 고쳐야 합니다.

---

## 업데이트 노트 · 안내 문구 관리 (supabase_v5.sql)

| 표 | 쓰임 |
|---|---|
| `update_notes` | 계산기별 '업데이트 됨 / 예정' 목록. 한 행이 한 항목 |
| `ui_texts` | 안내 문구. `key` 는 코드의 번역 사전 키와 같아야 함 |

- 불러오기는 `assets/uitext.js` 가 두 계산기 공용으로 합니다.
- 표가 없거나 비어 있으면 **코드에 있는 기본 문구**를 그대로 씁니다.
- 사전에 없는 `key` 는 무시합니다. 오타로 넣은 값이 조용히 쌓이거나 화면에
  새어나오는 걸 막기 위해서입니다.
- 어떤 언어로 한 줄도 안 적으면 그 언어의 목록은 **건드리지 않습니다.**
  한국어만 적어둔 상태에서 영어 화면이 텅 비는 걸 막습니다.

### 관리자에서 I18N 을 fetch 로 읽는 이유
`gecko-ui.js` 와 `crested-ui.js` 가 둘 다 최상위에서 `const I18N` 을 선언합니다.
`<script>` 로 같이 두면 *Identifier 'I18N' has already been declared* 로 관리자가
죽습니다. `admin-text.js` 는 파일을 텍스트로 받아 `I18N = { … }` 부분만 잘라
함수 안에서 실행합니다. 파일 전체를 실행하면 DOM 을 만지는 줄에서 멈춥니다.

### 계산기마다 문구 키가 다름
크레스티드에는 `comboNote`·`partialHint`·`vintageHint` 가 없고 대신
`optNote`·`fakeSuperNote` 가 있습니다. 관리자는 그 계산기 사전에 실제로 있는
키만 목록에 보여줍니다. 없는 키를 고쳐봤자 아무 데도 반영되지 않기 때문입니다.

---

## 9. 케어 관리 `/care/` (2026-08-02)

급여·청소·영양제 주기를 정해두면 오늘 할 일로 뜨고, 체중을 기록하고,
캘린더 파일로 내보내 폰에서 알림을 받는 화면입니다. 프리미엄 전용입니다.

### 파일

| 파일 | 하는 일 |
|---|---|
| `care/care-core.js` | 공통 순수 계산 — 종 프리셋, 고정 반복, `.ics`, 재고 예측, 주간 요약 |
| `care/care-routine.js` | 일일 달성·월~일 리듬·주 N회 진행률 계산 |
| `care/care-plan-store.js` | 세 반복 방식 중 하나만 저장하도록 계획 행 정규화 |
| `care/care-today-ui.js` | 오늘 할 일·연속 달성·주간 리듬 화면 |
| `care/care-plan-ui.js` | 계획 목록과 N일·요일·주 N회 설정 화면 |
| `care/care-app.js` | Supabase 읽기·쓰기와 연속 달성 서버 집계 호출 |
| `care/care-ui.js` | 탭 상태·공통 이벤트·화면 모듈 조합 |
| `care/animal-life-stage.js` | 생애 단계와 성체 부개체·모개체 후보·관계 검증 공통 규칙 |
| `care/care-animal-edit-route.js` | 개체 상세에서 기존 사진·개체 편집기로 안전하게 이동 |
| `care/animal-record-dialog.js` | 메모 빠른 기록의 입력·검증 대화상자 |
| `care/animal-calendar.js` | 최근 12주 기록을 날짜가 보이는 월별 달력으로 표현 |
| `care/care-health-ui.js` | 선택 개체와 건강·거식·증세 기록을 건강 탭에 연결 |
| `care/care.css` | 스타일 (계산기와 색만 맞추고 규칙은 따로) |
| `care/care-routine.css` | 달성 카드·7일 pill/dot·반복 설정 전용 스타일 |
| `care/manifest.webmanifest` | 홈 화면에 추가하면 앱처럼 뜨게 |
| `supabase_v16.sql` | `species` 칼럼 + 케어 표 3개 |
| `supabase_v31.sql` | 형질별 평가, 브리딩 목표, 페어링 단계와 소유권 검증 |
| `supabase_v32.sql` | 클러치 종 분리, 페어링 소유자·종 일치 검증 |
| `supabase_v33.sql` | 주 N회 반복·연속 달성 집계·사료 연결 소유권 보강 |
| `supabase_v34.sql` | 같은 날 체중 이력 보존·미래 해칭일 차단·공개 개체 카드 확장 (2026-08-03 라이브 적용) |
| `supabase_v37.sql` | 생애 단계·성체 부모 역할 검증·공개 체중 그래프와 브리더 페이지 |
| `supabase_v38.sql` | 이미 연결된 부모·페어링 개체의 미성숙 단계·성별·종·소유권 변경 차단 |
| `supabase_v39.sql` | 사용하지 않는 구형 공개 브리더 조회 함수 제거 |
| `supabase_v40.sql` | 새 개체 저장 시 공개 설정·생애 단계의 안전한 기본값 보정과 오류 안내 현지화 |
| `supabase_v41.sql` | 등록 화면의 첫 체중 이력과 체중 공개 설정을 개체와 함께 원자적으로 저장 |

### 브리딩 목표 저장 경계 `[구현]`

`breeding_projects`는 로그인한 사용자가 자기 행만 직접 읽고 쓰는 표입니다.
목표 JSON은 version 1, `line_fix`/`cross` 모드, 형질 수·중복·점수·크기를
DB에서 검증합니다. `pairings.project_id`를 연결할 때도 프로젝트와 페어링의
소유자와 종이 모두 같아야 하며, 프로젝트를 지우면 페어링 기록은 남고 링크만
해제됩니다. `animals.line_trait_scores`는 형질별 1~5점만 저장합니다.

브리딩 플래너는 레오파드·크레스티드·펫테일·볼파이톤을 지원합니다. 개체·페어링·
클러치 목록은 현재 종으로 분리되며, 클러치는 페어링을 연결하지 않아도 `species`를
반드시 저장합니다. 페어링을 연결하면 DB 트리거가 소유자와 종을 함께 검증합니다.

브리딩 관리와 생물 케어 스케줄 관리 화면은 한국어·영어·중국어·일본어를 지원합니다.
케어로그는 오늘·개체·계획·먹이·건강·요약뿐 아니라 개체 상세·공개 프로필·갤러리,
날짜·숫자·오류·ARIA·캘린더 내보내기까지 같은 언어 계약을 사용합니다. 화면과
공유 URL을 이동할 때도 `lang`을 유지합니다.

개체 생애 단계는 `baby_unknown`·`juvenile`·`subadult`·`adult` 네 값으로 저장합니다.
부모 후보는 성별만으로 판단하지 않습니다. 같은 소유자의 같은 종 중에서 본인을
제외하고, 부개체는 **성체 수컷**, 모개체는 **성체 암컷**만 보여줍니다. 같은 개체를
양쪽에 중복 지정할 수 없고, 이미 부모나 페어링에 연결된 성체를 아성체·준성체로
내리는 우회도 DB 트리거가 거부합니다. 기존 데이터 중 부모로 이미 참조된 개체는
성체로 보정했고, 나머지는 `베이비(미구분)`이므로 소유자가 실제 단계를 선택합니다.

### 왜 종을 가리지 않게 만들었나 `[구현]`

레오파드·크레스티드·펫테일·피그미다람쥐를 **한 화면**이 다룹니다. 급여·청소·
체중은 종이 달라도 구조가 같고, 다른 것은 기본 주기뿐입니다. 그 차이만
`care-core.js` 의 `SPECIES` 표로 뺐습니다.

> 새 종을 추가할 때는 `SPECIES` 에 한 덩어리만 더하면 됩니다.
> 화면·저장·캘린더는 손댈 것이 없습니다.

피그미 케어 앱(`K:\업무\...\Pygmy`)을 따로 만들려던 것을 여기로 합친 것입니다.
따로 지었으면 같은 화면을 두 벌 유지하게 됩니다. 그쪽 Flutter 코드는 쓰지
않았습니다 — 이 저장소는 빌드 도구가 없는 정적 HTML 이라 다시 쓰는 편이
빨랐습니다. 대신 **DB 스키마·주간 요약 규칙·리포트 안전 규칙**을 가져왔습니다.

### 알림을 캘린더에 맡긴 이유 `[구현]`

우리는 알림을 보내지 않습니다. `.ics` 파일에 반복 규칙(RRULE)을 담아 내보내면,
사용자 폰의 캘린더가 알아서 반복시키고 알림을 울립니다.

```
interval_days=3  →  FREQ=DAILY;INTERVAL=3
weekdays={1,4}   →  FREQ=WEEKLY;BYDAY=MO,TH
weekly_target=3  →  요일이 정해지지 않아 반복 일정에서는 제외
```

주 N회 계획은 먹이 소진 예측에는 `N/7`회/일로 반영하지만, 임의의 요일을
캘린더에 만들어내지는 않습니다. 계획 화면에서 이 차이를 안내합니다.

그래서 **주기적으로 도는 서버가 필요 없습니다.** 지금 이 사이트는 정적 자산만
있는 Worker 라 `fetch` 핸들러가 없고, 메일 발송 수단도 없습니다(v15 에서 비밀번호
재설정조차 운영자 수동 발급으로 바꾼 이유와 같습니다). 캘린더에 맡기면 그 제약을
전부 비켜갑니다. 덤으로 구글·애플·삼성 어디든 되고, 우리 서비스를 지워도
일정은 계속 울립니다.

`DTSTART` 에 시간대를 붙이지 않습니다. 시간대 없는 시각은 보는 사람의 현지
시각으로 읽히는데, 매일 아침 9시에 밥 주는 일에는 그게 맞습니다.

> 구글 캘린더 API 로 **양방향**(우리가 고치면 폰에 반영, 완료하면 캘린더에 반영)
> 을 하려면 `calendar.events` 범위가 필요하고, 그건 구글이 민감 범위로 분류해
> **OAuth 앱 검증**을 받아야 합니다. 수 주 걸리고, 검증 전에는 경고 화면과
> 사용자 100명 상한이 붙습니다. 애플·삼성 쓰는 사람은 혜택이 없고요.
> `.ics` 로 시작하고, 완료 반영 요구가 실제로 나오면 그때 올라가는 것을
> 권합니다. `[검토]`

### 반복 날짜는 시작일 기준 `[구현]`

"3일마다" 를 **시작일부터** 셉니다. "마지막으로 한 날부터 3일" 로 하면 늦게
했을 때 다음 날짜가 밀리는데, 캘린더는 그런 규칙을 표현할 수 없습니다. 두 곳이
다른 날을 가리키면 사용자는 어느 쪽을 믿어야 할지 모릅니다. 대신 건너뛴 것은
'밀림' 으로 따로 표시합니다.

### 케어 표는 RPC 를 안 씁니다 `[구현]`

기존 사육 기록은 `my_rows`/`save_row`/`delete_row` 를 거치지만, 케어 기록은
표에 직접 씁니다(표준 RLS). `my_rows` 가 표 전체를 jsonb 한 덩어리로 내려주는
구조라 매일 쌓이는 기록에는 못 쓰기 때문입니다. 그래서 케어는 로그인이
필수입니다. 자세한 이유는 `supabase_v16.sql` 머리말에 적어두었습니다.

**개체(`animals`)만은 예외로 계속 RPC 를 씁니다.** 브리딩 관리와 같은 데이터라,
두 화면이 서로 다른 규칙으로 같은 표를 만지지 않게 하려는 것입니다.

체중은 현재값을 덮어쓰는 프로필 필드가 아니라 `weight_logs`에 계속 추가되는
측정 이력입니다. 같은 날짜의 여러 측정도 `measured_at` 순서로 보존하고, 개체
상세에서 그래프와 개별 삭제 목록으로 확인합니다. 공개 카드에는 소유자가 별도로
허용했을 때만 최근 30개 측정 이력과 그래프를 내보냅니다. 부모 개체가 스스로
공개 상태일 때만 부모의 프로필 사진을 함께 보여주며, 브리더명 공개를 허용하면
브리더의 공개 개체를 모아 보는 별도 페이지로 연결합니다.

⚠️ v16 이 `animals` 에 **본인 읽기** 정책을 하나 엽니다. 케어 표의 RLS 가
"이 개체가 내 것인가" 를 `animals` 조회로 확인하는데, v6 이 그 표를 관리자
전용으로 잠가 두어 그대로면 아무도 케어 기록을 못 읽습니다. 조건은
`user_id = auth.uid()` 라 `my_rows` 로 이미 볼 수 있던 행과 같습니다.

### 리포트는 진단하지 않습니다 `[중요]`

주간 요약은 기록을 세어 보여주고 눈에 띄는 것만 짚습니다. **병명·치료·투약을
말하지 않고, '반드시·확실히' 같은 단정을 쓰지 않고, 기록이 적을 때 겁주지
않습니다.** 문구를 고칠 때 이 원칙을 함께 지켜 주세요.

`care-core.js` 의 `SAFETY_NOTE` 를 요약 화면 아래에 항상 붙입니다.

한 가지 실제로 걸렸던 것 — 여러 마리를 한꺼번에 보는 화면에서 체중 배열을
빈 값으로 넘겼더니 "체중 기록이 뜸합니다" 가 떴습니다. 매주 재고 있는 사람에게
없는 문제를 알린 셈입니다. 그래서 `weeklySummary(records, weights, …)` 의
`weights` 는 **`null` 이면 체중 이야기를 아예 꺼내지 않고**, 빈 배열이면
'기록 없음' 으로 봅니다.

### 아직 안 된 것 `[설계]`

- 클러치는 예상 부화일까지 기록하며 실제 부화 수·상태·새끼 연결은 아직 없습니다.
- 관리자 화면에 케어 탭이 없습니다. 지금은 `service='care'` 접속 로그만 쌓입니다.
- 서비스 워커가 없어 오프라인은 안 됩니다. 홈 화면 추가는 됩니다.
- iOS 는 설치 버튼을 띄울 수 없습니다. 공유 → '홈 화면에 추가' 안내가 필요합니다.
- 근친계수는 아직입니다. 개체 혈통(`parent_a`/`parent_b`)이 이미 있어 계산만
  붙이면 됩니다.

### 변경 이력에 추가

| 날짜 | 내용 |
|---|---|
| 2026-07-29 | 케어 관리 `/care/` 추가. `supabase_v16.sql`, 종 무관 구조, `.ics` 내보내기 |
| 2026-07-31 | `supabase_v31.sql` 추가. 형질별 평가와 인증 사용자 전용 브리딩 목표·페어링 연결 |
| 2026-08-02 | 브리딩 관리 4종 플래너·4개 언어 지원, `supabase_v32.sql` 클러치 종 분리 적용 |
| 2026-08-02 | 케어로그 전체 화면·공개 프로필·갤러리·캘린더 내보내기의 4개 언어 지원 |
| 2026-08-03 | `supabase_v37.sql`~`v39.sql` 적용. 4단계 생애 단계, 성체 부모 역할 검증, 공개 체중 그래프·부모 사진·브리더 페이지 |
| 2026-08-03 | `supabase_v40.sql` 적용. 체중 공개 설정이 없는 신규 개체도 비공개 기본값으로 저장하고 사용자 오류를 선택 언어로 안내 |
| 2026-08-03 | `supabase_v41.sql` 적용. 개체 등록에서 현재 체중과 체중 공개 여부를 받고 첫 체중 이력을 같은 트랜잭션으로 저장 |
