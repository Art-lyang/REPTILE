...
## 업데이트 노트 · 안내 문구 관리 (supabase_v5.sql)

| 표 | 쓰임 |
|---|---|
| `update_notes` | 계산기별 '업데이트 됨 / 예정' 목록. 한 행이 한 항목 |
| `ui_texts` | 안내 문구. `key` 는 코드의 번역 사전 키와 같아야 함 |

- 불러오기는 `assets/uitext.js` 가 두 계산기 공용으로 합니다.
- 표가 없거나 비어 있으면 **코드에 있는 기본 문구**를 그대로 씁니다.
- 사전에 없는 `key` 는 무시합니다. 오타로 넣은 값이 조용히 쌓이거나 화면에
  새어나오는 걸 막기 위해서입니다.
- 어떤 언어로 한 줄도 안 적으면 그 언어의 목록은 **건드리지 않습니다.**
  한국어만 적어둔 상태에서 영어 화면이 텅 비는 걸 막습니다.

### 관리자에서 I18N 을 fetch 로 읽는 이유
`gecko-ui.js` 와 `crested-ui.js` 가 둘 다 최상위에서 `const I18N` 을 선언합니다.
`<script>` 로 같이 두면 *Identifier 'I18N' has already been declared* 로 관리자가
죽습니다. `admin-text.js` 는 파일을 텍스트로 받아 `I18N = { … }` 부분만 잘라
함수 안에서 실행합니다. 파일 전체를 실행하면 DOM 을 만지는 줄에서 멈춥니다.

### 계산기마다 문구 키가 다름
크레스티드에는 `comboNote`·`partialHint`·`vintageHint` 가 없고 대신
`optNote`·`fakeSuperNote` 가 있습니다. 관리자는 그 계산기 사전에 실제로 있는
키만 목록에 보여줍니다. 없는 키를 고쳐봤자 아무 데도 반영되지 않기 때문입니다.
