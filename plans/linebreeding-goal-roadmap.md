...
이 문서는 구현 당시의 계약과 검증 기준을 보존하는 설계 기록입니다. 아래 TODO의
Task 1~8은 모두 구현·검수·라이브 배포까지 완료되었습니다. 이후 실제 범위가 처음
계획보다 확장되어 다음 항목도 반영됐습니다.

- 레오파드뿐 아니라 크레스티드·펫테일·볼파이톤 라인 플래너 활성화
- 브리딩 관리 전체 한국어·영어·중국어·일본어 지원
- `supabase_v31.sql`의 프로젝트 보안에 이어 `supabase_v32.sql`로 클러치 종 분리
- 실제 등록 새끼를 프로젝트 선발 후보와 다음 단계에 연결
- 공개 계산기에서는 계획 도구를 빼고 브리딩 관리의 분석 탭으로 이동

현재 상태와 다음 작업은 `docs/PROJECT_STATUS.md`를 기준으로 합니다. 아래의
“레오파드만 활성”, “다른 종 준비 중”, “최신 v30” 문구는 최초 MVP 범위를 설명하는
역사적 내용이며 현재 구현 상태가 아닙니다.
...
- 공개 모프 계산기는 계산기 역할만 유지하고, 장기 계획은 생물 케어 스케쥴 관리/브리딩 관리에서 다룬다.
- 레오파드게코로 먼저 완성하되 크레스티드·펫테일 등으로 확장 가능한 구조를 만든다.
