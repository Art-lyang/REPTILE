/* =============================================================================
   ...
   **통째로 갈아치웁니다.**
   ...
   ---------------------------------------------------------------------------
   그럼 코드는 왜 같이 고치나 — 두 가지가 DB 에 없습니다
   ---------------------------------------------------------------------------
   (1) 계열(line). morphs 표에 칼럼이 없어서 applyMorphRows 가 코드에서 가져옵니다.
   ...
   (2) 동반 유전자(implies). 이것도 칼럼이 없습니다. 그런데 계열과 달리
   **확률에 직접 영향을 줍니다.**

       const builtinImplies={}; POLY.forEach(p=>{ if(p.implies) builtinImplies[p.id]=p.implies; });

   레드데빌은 벨 알비노를 물고 있는 라인입니다. 이 연결이 없으면
   레드데빌을 노멀과 붙였을 때 최소 50% 가 het 벨인데 화면에는 0% 로
   나옵니다. 틀린 확률이 오류 없이 조용히 나오는 쪽이라 더 위험합니다.

   gecko-core.js 의 reddevil 항목이 이 연결을 들고 있습니다.

       {id:'reddevil', line:'tangerine', implies:{bell:'het'}, ...}

   'mm'(비주얼) 이 아니라 'het'(보인자) 입니다. 벨 알비노는 열성이라
   라인 안에 겉으로 안 드러나는 보인자가 섞여 있고, 레드데빌이라고 전부
   비주얼 알비노는 아니기 때문입니다. 모든 레드데빌에 대해 확실히 말할 수
   있는 선이 '최소 보인자' 입니다.

   그래서 implies 는 **최소값** 으로 다룹니다. 계산기에서 레드데빌 칩을
   켜면 벨 알비노가 het 으로 잡히고, 이유가 칩 아래에 적힙니다.
     · 내 개체가 눈으로 봐서 알비노면 → 벨 알비노를 '비주얼' 로 올리세요.
       경고는 안 뜹니다. 어긋난 게 아니라 더 진한 경우니까요.
     · 벨을 아예 안 물고 있으면 → 벨 알비노 칩을 끄세요.
       그때는 확률이 실제보다 낮다고 알려줍니다.
   ...
   ---------------------------------------------------------------------------
     ...
     레드데빌  붉은 발색 라인 — 단, 벨 알비노를 물고 있습니다(위 참고).
               붉은 발색 자체는 다인자라 계산 대상이 아니지만,
               딸려 있는 벨 알비노는 열성 유전자라 계산 대상입니다.
