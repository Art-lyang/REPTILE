/* =============================================================================
   supabase_v23.sql — 레오파드 라인브리딩 15종 추가
   -----------------------------------------------------------------------------
   ...
   그래서 15종 전부 kind='poly' 이고, combos 표에는 아무것도 넣지 않습니다.
   확률 계산에 끼어들지 않습니다.
   ...
   계열 없음 : GT · GG · GCT.M · GCT.S · BMD · 스모그 · 오렌지 스모그
   ...
   ※ '차콜카본다크만다린' 은 넣지 않습니다. 하나의 라인이 아니라
      차콜 + 카본 + 다크 + 만다린 을 붙여 쓴 이름이고, 넷 다 이미 따로
      있습니다. 계산기에서 네 개를 같이 누르시면 됩니다.
      항목으로 만들면 같은 개체를 두 방법으로 고를 수 있게 되고,
      그때 결과에 찍히는 이름이 서로 달라집니다.
   ============================================================================= */
...
select v.id, 'poly', 'poly', v.ko, v.en, v.zh, v.ja, false, v.color, base.n + v.ord, true
  from base, (values
    ...
    -- 붉은 계열
    ('crimson',       '크림슨',     'Crimson',        '深红', 'クリムゾン',         '#B02B26', 11),
    ('bordeaux',      '보르도',     'Bordeaux',       '波尔多', 'ボルドー',         '#7B2233', 12),
    ('darktangerine', '다크텐져린', 'Dark Tangerine', '暗橘', 'ダークタンジェリン', '#C4652A', 13),
    -- 알비노를 물고 있는 라인 (implies 는 코드에 있습니다)
    ('smog',       '스모그',        'Smog',        '烟灰',   'スモッグ',           '#6E6862', 14),
    ('orangesmog', '오렌지 스모그', 'Orange Smog', '橙烟灰', 'オレンジスモッグ',   '#B2743C', 15)
...
/* ── 2-0. 잘못 들어간 합성 항목 정리 ────────────────────────────────────
   이 SQL 의 이전 판(16종)에는 'ccdm'(차콜카본다크만다린) 이 들어 있었습니다.
   하나의 라인이 아니라 네 형질을 붙여 쓴 이름이라 뺐습니다.
   이전 판을 이미 돌리셨으면 이 줄이 지워줍니다. 안 돌리셨으면 아무 일도
   일어나지 않습니다. */
delete from public.morphs where id = 'ccdm';


/* ── 2. 확인 ─────────────────────────────────────────────────────────────
   15줄이 나오고 kind 가 전부 poly 여야 합니다. */
...
/* v22 까지 다 돌렸으면 27 + 15 = 42 가 나와야 합니다.
   (기존 24 + v22 의 3 + v23 의 15) */
