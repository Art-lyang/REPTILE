create or replace function public.care_is_public_animal_photo(
  p_object_name text,
  p_owner_id text
)
returns boolean
...
revoke all on function public.care_is_public_animal_photo(text,text) from public;
grant execute on function public.care_is_public_animal_photo(text,text) to anon, authenticated;
...
create policy ap_read on storage.objects
  ...
  using (
    ...
    and (
      ...
      or public.care_is_public_animal_photo(
...
select public.care_is_public_animal_photo(
  'not-a-real-object',
  'not-a-real-owner'
